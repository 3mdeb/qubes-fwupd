...
METADATA_URL_ASC = "https://fwupd.org/downloads/firmware.xml.gz.asc"
...
class DownloadData(FwupdVmCommon):
    ...
    def _download_metadata_asc(self):
        """Download metadata gpg signature"""
        if self.custom_url is None:
            metadata_url = METADATA_URL
        else:
            metadata_url = self.custom_url
        ...
        if p.returncode != 0:
            raise Exception("fwudp-qubes: Downloading metadata file failed")
        if not os.path.exists(f"{self.metadata_file}.asc"):
            raise FileNotFoundError(
                "fwudp-qubes: Downloaded metadata file does not exist"
            )
    ...
    def download_updates(self, url, sha):
        ...
        update_path = os.path.join(FWUPD_UPDATEVM_UPDATES_DIR, self.arch_name)
        cmd_update = [
            "wget",
            "-O",
            update_path,
